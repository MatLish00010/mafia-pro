...
import type { AddUser } from "@/requests/user/types/AddUser.ts";
import type { EditUser } from "@/requests/user/types/EditUser.ts";
...
export const addUser = async (body: AddUser) =>
	supabase
		...
		.insert([
			{
				nick: body.nick,
				data_birthday: body.data_birthday
					? format(new Date(body.data_birthday), "y-MM-d")
					: null,
				first_visit: body.first_visit
					? format(new Date(body.first_visit), "y-MM-d")
					: null,
...
export const editUser = async (dto: EditUser) =>
	supabase
		...
		.update({
			nick: dto.nick,
			data_birthday: dto.data_birthday
				? format(new Date(dto.data_birthday), "y-MM-d")
				: null,
			first_visit: dto.first_visit
				? format(new Date(dto.first_visit), "y-MM-d")
				: null,
