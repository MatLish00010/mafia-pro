...
export const getGamesWithDetails = async () =>
  supabase
    ...
    .then(res => {
      ...
      data.forEach(game => {
        game.game_details.forEach(detail => {
          ...
          if (detail.removed) {
            rating[nick].removed = sumTwoNotIntegerNumbers(REMOVED, rating[nick].removed);
          }
          ...
          rating[nick].sum = formatFromThousand(
            formatToThousand(rating[nick].breakLose) +
              ...
              formatToThousand(rating[nick].ratingByRoles.SHERIFF.wills.positive) +
              formatToThousand(rating[nick].ratingByRoles.BLACK.wills.positive) +
              //
              ...
              formatToThousand(rating[nick].ratingByRoles.SHERIFF.wills.negative) +
              formatToThousand(rating[nick].ratingByRoles.BLACK.wills.negative) +
              //
