...
const FirstKilled = ({players, onSubmit, defaultValues}: Props) => {
  ...
  const prevSubmit = (data: DataForm) => {
    ...
    if (typeof data.firstKilledPosition === 'number' && typeof data.bonuses === 'number') {
      onSubmit({firstKilled: data.firstKilledPosition, bonuses: data.bonuses});
    }
  };
                  ...
                  <FormItem>
                    <FormControl>
                      <Input
                        placeholder="0"
                        {...field}
                        value={firstKilledPositionWatch === index ? field.value : 0}
